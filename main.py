...
def createHelpPage(pageNum=0):
    helpData = {
        "Cheat Commands": {
            ...
            "</spamreaction_with_bots:123456789012345678>": "Spam reaction using bots."
        },
        "Other Commands": {
...
@bot.slash_command(description="Spam reaction using bots")
async def spamreaction_with_bots(ctx, botnames: str, roomcode: str, amount: int, reaction: str, intensity: int):
    await ctx.defer(ephemeral=True)
    if checkBlacklist(str(ctx.author.id)):
        await ctx.respond("**You're currently in the blacklist, you can't use any command except someone clears the blacklist.**")
    else:
        ...
        if rdata:
            ...
            botnames_list = botnames.split(',')
            if len(botnames_list) < amount:
                await ctx.respond("Not enough bot names provided. Please provide at least {} bot names.".format(amount))
                return

            # Adding bots
            success = 0
            for i in range(amount):
                playername = botnames_list[i]
                fakeip = f"{str(random.randint(100, 255))}.{str(random.randint(100, 255))}.{str(random.randint(100, 255))}.{str(random.randint(100, 255))}"
            ...
            if success > 0:
                await ctx.respond(f"Successfully Added {success} Bots to `{roomcode}`. Spamming reactions now...")
                for botname in botnames_list[:success]:
                    for _ in range(amount):
                        threading.Thread(target=react, args=(roomhash, hostid, botname, reactions[reaction], intensity)).start()
                        await asyncio.sleep(0.1)
            else:
                await ctx.respond("Failed to add any bots.")
        else:
            await ctx.respond("Invalid Room Code!")
